...
export default function UploadPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!isLoggedIn) {
      openLoginModal('Save State Ingestion');
      e.target.value = '';
      return;
    }
    if (e.target.files && e.target.files[0]) {
    ...
    <div className="min-h-screen bg-[#f4f1ea] text-black font-serif pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white border-2 border-black p-4 mb-6 shadow-sm">
        ...
        <div className="flex items-center gap-2">
            {isLoggedIn && currentCoach && (
              <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border-2 border-emerald-800 text-emerald-950 text-xs font-bold uppercase font-sans shadow-2xs">
                <ShieldCheck className="w-4 h-4 text-emerald-700 shrink-0" />
                <span>Coach: <strong>{currentCoach.coach_name}</strong></span>
              </div>
            )}
            ...
            <button
              ...
              disabled={isSaving || (isLoggedIn && Boolean(parsedGame) && scheduleStatus.isAllPlayed && !allowOverwrite)}
              style={{ 
                backgroundColor: !isLoggedIn 
                  ? '#1c1917' 
                  : (parsedGame && scheduleStatus.isAllPlayed && !allowOverwrite) 
                  ? '#991b1b' 
                  : '#16a34a', 
                color: '#ffffff' 
              }}
              ...
              title={!isLoggedIn ? "Coach login required" : parsedGame ? "Save parsed game stats to Supabase" : "Click to select a save state file and submit"}
            >
              {!isLoggedIn ? (
                <>
                  <Lock className="w-4 h-4 md:w-5 md:h-5 text-yellow-400 shrink-0" />
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-4">
          <div className="bg-white border-2 border-black p-5 shadow-sm relative overflow-hidden">
            {!isLoggedIn && (
              <CoachLockOverlay
                title="COACH LOGIN REQUIRED"
                description="Save state uploading and database ingestion requires an active coach login. Please sign in with your coach profile to upload game states."
                buttonText="Sign In to Upload State"
                loginContext="Save State Ingestion"
              />
            )}
